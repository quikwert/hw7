...
public class Puzzle {
	...
	//variable for printout
	static String puzzle;


	public static void main (String[] args) {
		if(args.length != 3) throw new RuntimeException("Too many arguments! Must be 3!");
		for (String arg : args) {
			if (!arg.matches("[A-Z]+")) {
				throw new RuntimeException("Only Upper Case Latin alphabet letters are permitted!");
			}
		}
		puzzle = args[0] + " + " + args[1] + " = " + args[2];
	...
	//	}	
		...
		int solutions = backtrack(new byte[10], (byte)0, coefMap, 0, 0, mostSignificantLetters, digits);
	...
	static void printout(HashMap<Character, Integer> digits, HashMap<Character, Integer> map){
		StringBuffer sb = new StringBuffer();
		for(int i = 0; i < puzzle.length(); i++){
			if(map.containsKey(puzzle.charAt(i))) sb.append(digits.get(puzzle.charAt(i)));
			else sb.append(puzzle.charAt(i));
		}	
		System.out.println(sb.toString());
	}

	static int backtrack(byte[] used, byte idx, HashMap<Character, Integer> coefMap, int sum, int solutions, char[] MSL, HashMap<Character, Integer> digits){
		if(idx == coefMap.keySet().size()){
			if( sum == 0 ){
				printout(digits, coefMap);			
				return solutions + 1;
			}
			return solutions;
		}
		...
		for(int i = 0; i < 10; i ++){
			...
			used[i]=1;	
			digits.put(letter,i);
			solutions = backtrack(used, (byte)(idx+1), coefMap, sum + coefMap.get(letter)*i, solutions, MSL,digits);
			used[i]=0;
		}	
		return solutions;
	}
